...
def new_sentence():
    variable = findall_last_words[0]
    view = findall_last_words[1]
    paragraph = findall_last_words[2]
    here = findall_last_words[3]
    mistake = findall_last_words[4]
    text = findall_last_words[5]
    whitespaces = findall_last_words[6]
    nbr = findall_last_words[7]
...
def splitted(a):
    ...
    splitteds = re.split(a, message)
    # capitalize first letter or sentences
    capital_letters = [i.capitalize() for i in splitteds]
    # join all parts back
...
findall_last_words = findall_last_word('\s(\w+)?\.', initial_message)

splitteds = splitted('(\.\s+)')
